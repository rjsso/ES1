...
public class FileReaderTest {
	...
	@Test
	public void saveRuleValuesTest(){
		String file =  "src/jUnitTests/rules.cf";
		
		List<Rule> rulesList = reader.getRulesFromFile(file);
		List<String> rulesValues = new ArrayList<String>();
		
		for(int i=0; i<rulesList.size();i++) {
			rulesValues.add("0");
		}
		
		reader.saveRuleValues(rulesValues,file,rulesList);
		
		List<Rule> test = reader.getRulesFromFile(file);
		assertNotNull(test);
	}
